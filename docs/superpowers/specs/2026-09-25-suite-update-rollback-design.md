...
8. 链完整性不能证明会话顺序（第三份独立审查，P2）：§18.10 只比较本机当前状态与目标会话的后状态，却声称“保证回滚只能按逆序逐级进行”。反例：用 `update --to` 形成 A→B→C→B，会话依次为 s1（A→B）、s2（B→C）、s3（C→B）。本机当前是 B，等于 s1 的后状态，于是 `rollback --to s1` 能通过校验，跳过 s3、s2，并恢复 s1 之前的配置快照。修订方向（二选一，须在重新提交前决定）：
   - 链完整性增加顺序条件：目标会话之后的所有会话，状态都必须是 `rolled_back` 或 `failed`（没有替换过任何路径）；否则拒绝，返回 1，并列出需要先回滚的会话；
   - 或者禁止产生这种链：`update --to` 的目标版本与本机当前版本相同，或与任何未回滚的 `success` 会话的前版本相同时，拒绝执行。
   无论选哪种，§18.7 都要增加重复版本的用例：A→B→C→B 之后，`rollback --to s1` 被拒绝，二进制、脚本和配置的字节都不变。
...
这条检查单独**不足以**保证回滚按逆序逐级进行：本机状态相同，不代表中间没有其他会话（反例与修订方向见 §0 第 8 条）。
